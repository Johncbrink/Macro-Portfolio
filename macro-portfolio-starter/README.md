...
## Data
- `src/aggregate_dti_from_csv.py` expects six CSVs in `data/` with columns **DATE, VALUE** and exact file names:
  - `TCMDO.csv` `GDP.csv` `CMDEBT.csv` `DSPI.csv` `BCSNSDODNS.csv` `CP.csv`

> Tip: keep `data/` in the repo for now (fine for public FRED), or add it to `.gitignore` if you prefer a clean public repo.
